...
func AddFlags(flags *pflag.FlagSet) {
	flags.Bool("accessToken.enabled", false, "enabled configures the access token source for GetAccessToken requests")

	flags.String("accessToken.source.file.tokenpath", "", "tokenPath is the path to the source jwt token")
	flags.String("accessToken.source.clientCredentials.issuer", "", "issuer specifies the URL for the issuer for the token request. The Issuer must support OpenID discovery to discover the token endpoint.")
	flags.String("accessToken.source.clientCredentials.clientID", "", "clientID is the client credentials id which is used to retrieve a token from the issuer. This attribute also supports a file path by prefixing the value with `file://`. example: `file:///var/secrets/client-id`")
	flags.String("accessToken.source.clientCredentials.clientSecret", "", "clientSecret is the client credentials secret which is used to retrieve a token from the issuer. This attribute also supports a file path by prefixing the value with `file://`. example: `file:///var/secrets/client-secret`")

	flag.String("accessToken.exchange.issuer", "", "issuer specifies the URL for the issuer for the exchanged token. The Issuer must support OpenID discovery to discover the token endpoint")
	flag.String("accessToken.exchange.grantType", "urn:ietf:params:oauth:grant-type:token-exchange", "grantType configures the grant type")
	flag.String("accessToken.exchange.tokenType", "", "tokenType configures the token type")

	flag.Duration("accessToken.expiryDelta", 10*time.Second, "sets the early expiry validation for the token")
}
